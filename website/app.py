...
def prediction(lst):
    filename = 'model\RandomForestRegressor.pickle'
    with open(filename, 'rb') as file:
        model = pickle.load(file)
    pred_value = model.predict([lst])
    return pred_value

def prediction2(lst2):
    filename = 'model\LenearRegression.pickele'
    with open(filename, 'rb') as file:
        model = pickle.load(file)
    pred_value = model.predict([lst2])
    return pred_value

def prediction3(lst3):
    filename = 'model\DecisionTreeRegressor.pickele'
    with open(filename, 'rb') as file:
        model = pickle.load(file)
    pred_value = model.predict([lst3])
    return pred_value

def prediction4 (lst4):
    filename = 'model\svm.pickele'
    with open(filename, 'rb') as file:
        model = pickle.load(file)
    pred_value = model.predict([lst4])
    return pred_value

# RandomForestRegressor
@app.route('/RandomForestRegressor', methods=['POST', 'GET'])
...
# leanier regression
@app.route('/LenearRegression', methods=['POST', 'GET'])
def contact():
       pred_value = 0
       if request.method == 'POST':
        ram = request.form['ram']
...
#decision tree
@app.route('/DecisionTreeRegressor', methods=['POST', 'GET'])
def desTree():
       ...
       return render_template('index3.html', pred_value=pred_value)
...
def svm():
         ...
         return render_template('index4.html', pred_value=pred_value)
